import { ctx, drawspeed, iconRadius } from './connector';

export function showIcon(x, y, iconId, href) {
  const canv = document.querySelector('#tagCanvas');
  $('body').append(`<div class="socialIcon" id="${iconId}"><a class="fab ${iconId}"></a></div>`);
  const icon = $(`#${iconId}`);
  icon.css({
    left: `${x - iconRadius}px`, top: `${y - iconRadius}px`, width: `${iconRadius * 2}px`, height: `${iconRadius * 2}px`,
  });
  icon.animate({ color: '#ffffff' }, 300);
  const oldCanv = cloneCircle(canv, x, y);
  ...
  icon.hover(() => {
    ...
    icon.stop(true, false);
    icon.animate({ color: '#ffffff' }, 300);
    fillCircle(x, y, 0, 'rgba(0, 0, 0, 0.3)', iconRadius * 0.9, () => {
